...
# ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
        ...
        st.info("İş Bankası, Akbank, QNB, Ziraat vb. bankalardan indirdiğiniz Excel ekstresini buraya yükleyin.")
        ...
        if bank_file:
            if st.button("🚀 Banka Ekstresini İşle ve Eşleştir", use_container_width=True, type="primary"):
                with st.spinner("Banka formatı analiz ediliyor ve sütunlar eşleştiriliyor..."):
                    try:
                        df_raw = pd.read_excel(bank_file)
                        baslik_satiri = 0
                        
                        # 1. BAŞLIK SATIRINI KESİN BULMA
                        for i in range(min(30, len(df_raw))):
                            satir_metni = " ".join([str(val).upper() for val in df_raw.iloc[i].values])
                            if any(k in satir_metni for k in ["TUTAR", "BORÇ", "BORC", "ALACAK", "BAKİYE", "NET"]) and any(k in satir_metni for k in ["AÇIKLAMA", "İŞLEM", "ISLEM", "DETAY"]):
                                baslik_satiri = i + 1
                                break
                        ...
                        df_bank.columns = [str(col).strip().replace('*', '').replace('\n', ' ').upper() for col in df_bank.columns]
                        
                        # 2. AKILLI VE KATI SÜTUN EŞLEŞTİRME (İŞ BANKASI "HE, FA" HATASINI ÇÖZEN KISIM)
                        tarih_kolonu = next((col for col in df_bank.columns if any(k in col for k in ["TARİH", "TARIH", "ZAMAN"])), None)
                        
                        tutar_kolonu = next((col for col in df_bank.columns if col in ["TUTAR", "TUTARI", "İŞLEM TUTARI", "ISLEM TUTARI", "NET TUTAR", "NET"]), None)
                        if not tutar_kolonu:
                            tutar_kolonu = next((col for col in df_bank.columns if "TUTAR" in col and "KURS" not in col), None)
                            
                        borc_kolonu = next((col for col in df_bank.columns if "BORÇ" in col or "BORC" in col or "ÇIKAN" in col), None)
                        alacak_kolonu = next((col for col in df_bank.columns if "ALACAK" in col or "GİREN" in col or "YATAN" in col), None)
                        
                        aciklama_kolonu = None
                        # Öncelik 1: Tam isimde "AÇIKLAMA" veya "DETAY" geçenler (İş Bankası için hayat kurtaran adım)
                        for col in df_bank.columns:
                            if col in [tarih_kolonu, tutar_kolonu, borc_kolonu, alacak_kolonu]: continue
                            if any(k in col for k in ["AÇIKLAMA", "ACIKLAMA", "DETAY"]):
                                aciklama_kolonu = col
                                break
                                
                        # Öncelik 2: Bulamazsa "İŞLEM ADI", "HAREKET" kelimelerini arasın
                        if not aciklama_kolonu:
                            for col in df_bank.columns:
                                if col in [tarih_kolonu, tutar_kolonu, borc_kolonu, alacak_kolonu]: continue
                                if any(k in col for k in ["İŞLEM ADI", "ISLEM ADI", "HAREKET"]):
                                    aciklama_kolonu = col
                                    break
                        
                        if not aciklama_kolonu:
                            st.error("Hata: Banka ekstresinde 'Açıklama' sütunu bulunamadı.")
                        elif not tutar_kolonu and not (borc_kolonu and alacak_kolonu):
                            st.error("Hata: Banka ekstresinde 'Tutar' veya 'Borç/Alacak' sütunları algılanamadı.")
                        else:
                            # 3. ÇİFT SÜTUNLU (BORÇ/ALACAK) EKSTRELERİ BİRLEŞTİRME
                            if not tutar_kolonu and borc_kolonu and alacak_kolonu:
                                df_bank['AKILLI_TUTAR'] = 0.0
                                df_bank[borc_kolonu] = df_bank[borc_kolonu].apply(tutar_temizle)
                                df_bank[alacak_kolonu] = df_bank[alacak_kolonu].apply(tutar_temizle)
                                
                                for idx, row in df_bank.iterrows():
                                    al = row[alacak_kolonu]
                                    bo = row[borc_kolonu]
                                    if al > 0: df_bank.at[idx, 'AKILLI_TUTAR'] = al
                                    elif bo > 0: df_bank.at[idx, 'AKILLI_TUTAR'] = -bo
                                tutar_kolonu = 'AKILLI_TUTAR'
                            else:
